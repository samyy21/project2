...
import com.paytm.pgplus.upipsphandler.service.impl.EncryptedParamsRequestServiceHelper;
...
public class TheiaResponseGeneratorHelper {
    ...
    public StringBuilder encryptedResponse(TransactionResponse response, boolean isAES256Encrypted,
        ...
        if (merchantPreferenceService.isChecksumEnabled(response.getMid())
                && !(TheiaConstant.ResponseConstants.ResponseCodes.CHECKSUM_FAILURE_CODE.equals(response.getResponseCode()))) {
            try {
                String checksum;
                String merchantKey;
                if (isAES256Encrypted) {
