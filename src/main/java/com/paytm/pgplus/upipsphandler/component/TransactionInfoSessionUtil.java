...
import com.paytm.pgplus.upipsphandler.enums.SavedCardType;
import com.paytm.pgplus.upipsphandler.model.*;
import com.paytm.pgplus.upipsphandler.service.IMerchantOfferDetailsService;
...
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.ExtraConstants.PPBL_BALANCE_FAIL_MSG;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.ExtraConstants.PPBL_INSUFFICIENT_BALANCE;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.RequestParams.ADD_MONEY;
import static com.paytm.pgplus.upipsphandler.constants.TheiaConstant.RequestTypes.*;
import static com.paytm.pgplus.upipsphandler.enums.ERequestType.SUBSCRIPTION;
...
public class TransactionInfoSessionUtil {
    ...
    private void mapConsultFeesResponse(final WorkFlowResponseBean responseData, final TransactionInfo txnInfo) {
        if (BeanParamValidator.validateInputObjectParam(responseData.getConsultFeeResponse())
            ...
            for (final Map.Entry<EPayMethod, ConsultDetails> entry : responseData.getConsultFeeResponse()
        ...
        try {
            ...
            if (StringUtils.isNotBlank(requestData.getChannelId())) {
                ...
                final ReadableUserAgent userAgent = parser.parse(requestData.getRequest().getHeader(
                        TheiaConstant.RequestHeaders.USER_AGENT));
